...
Last updated: 2026-08-07 (Asia/Shanghai) — M2 学习状态云同步 Phase 1-5 完成（上传/下载/merge/debounce/状态/验证） — 产品化方向确认（账户与云同步、会员边界、发布打磨；部署暂缓待域名）；M1 账户与认证完成：邮箱+密码登录、Prisma+SQLite、JWT 会话、登录/注册页、账户菜单集成；next priority: M2 学习状态云同步（进度/记忆/收藏/模考）
