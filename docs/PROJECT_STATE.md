...
Last updated: 2026-08-16 (Asia/Shanghai) — Agent 改写提案一键应用完成（两房间智能合并 + 单槽撤销 + 应用后保持未确认 + 确定性参考句应用入口） — M2 同步冲突可见性 + 用户裁决完成（SyncConflict 检测/存储/解决 + 账户面板 UI + 服务端 FSRS upsert 时间戳守卫 + lastMergeAt 基线） — 错题中心升级为三层能力中心（客观错题 + 结构薄弱 + FSRS 高危，复用 learning-memory 确认记录与 fsrsState，无新存储） — M2 学习状态云同步 Phase 1-5 完成（上传/下载/merge/debounce/状态/验证） — 产品化方向确认（账户与云同步、会员边界、发布打磨；部署暂缓待域名）；M1 账户与认证完成：邮箱+密码登录、Prisma+SQLite、JWT 会话、登录/注册页、账户菜单集成；next priority: M2 学习状态云同步（进度/记忆/收藏/模考）
