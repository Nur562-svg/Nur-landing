...
- There is no database, general content-management UI, authentication, server-side learner persistence, or real learner analytics. Personal exam structure, confirmed attempts, A/B preferences, review tasks, and Course Builder approval are narrow, validated browser-local state. The two server boundaries are the stateless NUR Agent and Course Builder Route Handlers.
...
Do not treat these limitations as permission to fill the product with placeholders. The evidence-gated material-admission milestone, official TCM material-pack v1, base-pack-independent private analysis, private learning actions, the Qwen-powered bounded Agent, the 33-item assessment bank with a 53/100-point mock half-paper, and the wrong-question center with weak-KP weekly-plan reflow are complete. The next narrow priority is to confirm B1/B2 semantics before the remaining mock-exam shortfalls can be closed, or to complete the Section 8.5 platform Agent browser QA. A future admitted-record-to-Course-Builder selection bridge still needs its own use and one-time transfer approvals; official compilation, objective-question answer review, physiology discovery, PDF/PPT/image/OCR breadth, authentication, database, deployment, memberships, and billing remain separate and deferred.
